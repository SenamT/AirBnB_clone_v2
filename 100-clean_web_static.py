...
def do_clean(number=0):
    """this will format the input and clean the remote"""
    n = 1
    if int(number) != 0:
        n = int(number)
...
@task
def deploy():
    """this is a fabric script that will create and distribute an archive"""
    file_name = do_pack()
...
def do_deploy(archive_path):
    """this is a fabric script that will distribute an archive to web servers"""
    try:
